...
const AD_KEY_300x250 = 'f97945650890088425224b7f3d3abb56';
          ...
          <div className="relative flex max-h-[62dvh] flex-col overflow-y-auto bg-[#05050A] p-3 sm:p-6 md:p-8 lg:max-h-none lg:overflow-visible lg:p-9 custom-scrollbar">
            ...
            <div className="mt-3 flex gap-2 sm:mt-5 sm:flex-row sm:gap-3 lg:mt-auto lg:pt-6">
              <button
                onClick={() =>
                  window.open(
                    `https://1Flex.sc/tv/${seriesId}/${selectedSeason}/${episode.episode_number}`,
                    "_blank"
                  )
                }
                ...
                <div className="space-y-3 md:space-y-5">
                  ...
                  <HorizontalScroller>
                    {filteredCrew.map((person, idx) => {
                      // Get role-specific URL
                      const getRoleHref = () => {
                        const slug = slugify(person.name, person.id);
                        if (person.job === 'Director') return `/director/${slug}`;
                        if (person.job === 'Writer') return `/writer/${slug}`;
                        if (person.job === 'Screenplay') return `/writer/${slug}`;
                        if (person.job === 'Story') return `/writer/${slug}`;
                        if (person.job === 'Producer') return `/producer/${slug}`;
                        if (person.job === 'Executive Producer') return `/producer/${slug}`;
                      ...
                      return (
                        <Link href={getRoleHref()} key={`${person.id}-${idx}`} className="group flex flex-col w-28 sm:w-32 md:w-36 shrink-0 bg-gradient-to-b from-[#0F0F1A] to-black rounded-xl border border-[#1F2937] overflow-hidden hover:border-[#E50914]/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
